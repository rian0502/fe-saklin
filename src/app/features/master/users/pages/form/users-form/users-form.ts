...
export class UsersForm {
  ...
  readonly form = this.formBuilder.nonNullable.group({
    ...
    // Comma-separated in the UI; converted to/from MasterUser.roles (string[])
    // at the service boundary in loadUser()/submit().
    roles: [''],
  });
  ...
  constructor() {
    ...
    this.destroyRef.onDestroy(() => this.userService.clearSelectedUser());
  }
  ...
  submit(): void {
    ...
    const payload: Partial<MasterUser> = {
      name: raw.name,
      email: raw.email,
      status: raw.status,
    ...
    const request$ = id
      ? this.userService.updateUser(Number(id), payload)
  ...
  private describeLoadError(error: unknown): string {
    ...
    return 'Something went wrong while loading this user.';
  }
